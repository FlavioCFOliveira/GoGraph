...
// raised the default to 30 minutes and rmp #2807 set it to 0, which DISABLES the
// reaper, so the pin is no longer what removes that rescue — an inherited default
// would remove it by itself. It is kept so these gates measure against a bound
// they state, instead of inheriting whatever the default becomes next; ten minutes
// sits far above these tests' budgets, which is all the property under test
// requires.
