...
10. [Disk exhaustion (ENOSPC)](#disk-exhaustion-enospc)
11. [Bulk-import publication parity (rmp #2466)](#bulk-import-publication-parity-rmp-2466)
12. [Read-only transaction isolation](#read-only-transaction-isolation)
13. [MVCC multi-session and concurrency coverage](#mvcc-multi-session-and-concurrency-coverage)
14. [Language-surface gaps (rmp #2462)](#language-surface-gaps-rmp-2462)
15. [Concurrency hypotheses chased](#concurrency-hypotheses-chased)
16. [Swarm, coverage, and cross-checking modes](#swarm-coverage-and-cross-checking-modes)
17. [Command-line usage](#command-line-usage)
18. [Reproduce, replay, and shrink](#reproduce-replay-and-shrink)
19. [Extending the harness](#extending-the-harness)
...
Two oracles added by rmp #2490 sit alongside those, both in the
`index-diversity` scenario. An **intersect-planner probe set**
(`index_intersect_probes.go`) issues a two-predicate conjunction over the two
BTREE-indexed properties — the only shape that reaches the planner's bitmap
composition (`cypher/index_intersect_plan.go`) and the budgeted per-conjunct
`RangeCount` / `RangeCountFrom` gate behind it — in an unbounded-above and a
bounded string spelling, each in its literal and parameterised form. Results are
verified against an independent client-side-filtered label scan; the physical
plan must carry the composed marker and must retain its mandatory residual
`Filter`; and the unbounded arm, whose two conjunct cardinalities are both exact,
must AND its parts in the order those cardinalities predict — the only place a
budgeted count's VALUE is observable at all. A single-property control must seek
through the same operator WITHOUT composing, so a composed marker cannot be
mistaken for "an index was used".

A pair of **recovery hydration arms** (`index_hydration_arms.go`) drives the
per-index deserialize-or-rebuild decision down BOTH branches deliberately, since
a hydrated index and a rebuilt one are by contract indistinguishable in their
answers and no result-level oracle can separate them. The instrument is the
engine-scoped population counter `cypher.Engine.RecoveredIndexPopulation`,
surfaced as `SimStore.RecoveredIndexPopulation`: the process-global
`store.recovery.indexes.*` metrics cannot attribute a decision to one reopen, and
the swarm runner builds engines concurrently. One arm forces a crossing of the
snapshot boundary with nothing committed after the checkpoint and requires every
registered index HYDRATED with zero node references backfilled; the other commits
one write to the indexed properties first and requires every index REBUILT and
that write reachable THROUGH the rebuilt indexes. Each arm carries an independent
substrate-level witness of its own precondition (an emptied WAL and zero replayed
ops; a non-zero replayed-op count), and a terminal gate requires both to have run.
...
quiescent view of the graph) and performs three independent families of check.
...
|---|---|---|
| Reachability | BFS, DFS, direction-optimised BFS, BiBFS and `BiBFSOn`, external-memory `extern.BFS` | The reachable **set** from a source (order-independent). Direction-optimised BFS additionally matches per-node **depths** against a from-scratch BFS; the bidirectional pair matches the returned path's **hop length**, never its identity, since the witness is not unique — and `BiBFSOn` is driven with an explicitly hoisted reverse CSR so the caller-supplied reverse path is covered too. |
| Components | WCC, `WCCParallel` | The partition, **up to relabelling** — serial and parallel judged against the same independent union-find reference. |
...
| SSSP / APSP | Dijkstra, Bellman-Ford, bidirectional Dijkstra in both its self-building and its hoisted-reverse (`…On`) form, A\*, Floyd-Warshall, Johnson, Dijkstra-APSP (the APSP family serial **and** parallel), the stateful `NewSSSP`, and the buffer-reuse `DijkstraInto` / `BellmanFordInto` / `AStarInto` | The **distance map**, not path identity; serial and parallel variants must agree exactly. The hoisted-reverse Dijkstra is judged **twice** — against the naive reference like its two siblings, and for *exact* cost parity against the self-building form, which is the only assertion that pins the caller-supplied reverse CSR. Negative weights and `ErrNegativeCycle` have their own arm (`search_negweight.go`), and an undersized caller buffer must yield `ErrBufferTooSmall`. |
| MST | Kruskal, Prim | The **total weight** plus spanning-forest validity, not the edge set. |
| Flow | max-flow (Dinic), Edmonds-Karp, push-relabel, Stoer-Wagner, min-cost max-flow | For the max-flow family the flow **value**, with max-flow = min-cut as a second invariant; for the global min cut the cut **weight**. Min-cost max-flow is driven through `MinCostMaxFlowCtx` — so its errors cannot be swallowed, which the non-context wrapper does — with that wrapper pinned beside it, in **both** cost regimes (all-positive, and forced-negative so the Bellman-Ford potential bootstrap runs), against an independent successive-shortest-paths / SPFA reference; plus a negative-cycle **optimality certificate** on the reference's own final residual, and `flow.ErrNegativeCycle` asserted by name on planted negative-cycle fixtures. |
| Matching | Hopcroft-Karp, Hungarian | Matching **cardinality** / assignment **total cost**, not the matching itself. |
| Euler | Hierholzer (directed), `HierholzerUndirected` | *Validated* trail against the **degree-parity** condition as the independent oracle: all-even-degree connected → a closed circuit using every edge exactly once; exactly-two-odd-degree → an open path between those two vertices; more than two odd vertices → `ErrNoEulerian`. The trail is not unique, so the covering invariant is validated rather than one canonical answer. |
| Centrality | betweenness — serial and parallel, unweighted and weighted; Closeness; Harmonic; Eigenvector; Katz; `PersonalisedPushPageRank` | Per-node value within a **measured** epsilon against from-definition references (Brandes for betweenness, all-pairs BFS for closeness and harmonic, power iteration for the spectral pair, the exact fixpoint for PPR), with a pinned worker count. Eigenvector, Katz and PPR each run in **both** the hand-written literal-options regime and the shipped `Default…Options` regime, under separate tolerances each derived from a recorded sweep; Katz's auto-alpha `0.85/(1 + maxInDegree)` is re-derived independently from the fixture, so the documented formula is pinned rather than trusted. |
| PageRank | one-shot PageRank; the stateful `PageRanker` | The rank vector within a convergence-aware epsilon, against an independent power-iteration reference matching the damping, dangling-mass redistribution, and teleport model. The stateful object is held additionally to **bit-identity** against `PageRankCtx` and to its documented buffer-aliasing contract (the `pagerank-ranker` scenario). |
...
| Diameter | `Diameter` (2-sweep / iFUB estimator) | The `(lo, hi, exact)` triple against the true diameter from an independent all-pairs BFS: `lo <= true <= hi` and `lo <= hi` always, and a claimed-`exact` result must satisfy `lo == hi == true`. |
| Triangles | `CountTriangles`, `CountTrianglesParallel` | The total **and** the per-node counts, by *exact* equality (they are integers, so no tolerance applies), against a reference that enumerates each triangle once by its minimum-id vertex; serial and parallel must agree bit-for-bit, the triangle monoid being order-independent integer addition. |
| External memory | `extern.BFS`, `extern.PageRank` | The semi-external algorithms, run over an mmap-backed `csrfile.Reader` of the same graph, against their in-memory equivalents: the reachable **set** for BFS, the rank **vector** for PageRank. |
...
**3. Context cancellation.** Every public **context-accepting** entry point of
`search`, `search/centrality`, `search/community`, `search/flow` and
`search/extern` — 58 of them — is driven under `context.Background()` and under
a context cancelled before the call, on the same cadence
(`internal/sim/search_ctx_cancel.go`, rmp #2489). The inventory is enumerated
from the **parameter type** with `go/ast` rather than from a `…Ctx` name suffix,
and a tripwire fails in both directions if the parsed set and the driven table
diverge — a name filter would have declared full coverage over 53 of the 58,
losing the three `*Into` primitives, `KShortestPathsLooplessCtxWithOpts` and the
`PageRanker.Run` method. Clauses assert **error identity** with `errors.Is`
(`ErrCycle`, `ErrNegativeCycle`, `ErrNoPath` and `ErrInvalidInput` are all
non-nil and none is a cancellation), and a separate precedence arm drives seven
rows whose input makes the entry point return a terminal sentinel, requiring
cancellation to outrank it. What the family does **not** claim is stated in
[docs/dst-feature-coverage.md](dst-feature-coverage.md#every-public-context-accepting-search-entry-point-under-cancellation-rmp-2489):
both regimes cancel *before* the call, so there is no promptness claim, and the
`goleak` teardown arm proves no leak rather than prompt joining.

## Scenario catalogue

A scenario is a named, self-contained configuration (seed, workload, fault
schedule, budget, mode, checks). All 55 scenarios that `DefaultRegistry`
(`internal/sim/catalogue.go`) builds have a row below, and `cmd/sim
--list-scenarios` prints exactly that set. Four further rows are listed here for
completeness but are **not** registry entries and are not printed by
`--list-scenarios` — `codec-matrix`, `txn-oversize`, `db-teardown` and
`checkpoint-cadence` are driven from their own tests rather than selected by
name.
...
| `index-diversity` | deterministic | Index-type diversity: a HASH (string), a BTREE (numeric), and a BTREE (string) index are created over an above-threshold graph (engaging the morsel-parallel backfill phase), then write churn + crash/recovery run while the thorough seek-vs-scan consistency check confirms each index agrees with its base data — for both kinds and both value types, including after recovery re-registers and re-populates them. The scenario also carries the access-path parity and plan-stability oracles: literal vs parameterised predicates (equality, range, `STARTS WITH`, `IN`-list) must agree on results and on the physical access path, and the fixed probe set must re-plan byte-identically after every crash/recovery; and the seek-result diversity oracle: bounded and half-open ranges, `STARTS WITH`, and `IN`-shaped predicates (list and `UNWIND` spellings), literal and parameterised, must reproduce an independent full-scan reference as id-multisets and counts, with a terminal non-vacuity assertion; and the schema-introspection oracle (rmp #2455): `SHOW INDEXES` and `db.indexes()` must reproduce the harness's model of the three declared indexes (name, kind, label, property) after the backfill, after every recovery, periodically, and at the end; and the statistics-regime oracle (rmp #2456): `CALL db.stats.refresh()` runs before the crash window (with a back-to-back throttle probe), after every recovery (the recovered engine must report zero tracked statistics pairs, then rebuild immediately on its fresh rate limiter), and at seed-chosen ticks — with the probe battery asserted result-identical across every refresh, refresh-correlated plan changes reported rather than failed, and a terminal non-vacuity gate (at least one rebuild that published statistics and at least one rate-limit refusal). Since rmp #2464 in-loop checkpointing is enabled, so the three index DEFINITIONS cross the SNAPSHOT boundary: a checkpoint truncates the WAL prefix that declared them, after which the post-recovery consistency and introspection checks are validating definitions loaded from the snapshot's `indexdefs.bin` and `indexes/` components rather than a replayed `CREATE INDEX`. A terminal gate asserts a non-zero checkpoint count. Since rmp #2490 the index CONTENTS cross it too: a recovered index is either HYDRATED from the snapshot's `indexes/<name>.bin` payload or REBUILT from the recovered graph, decided per index against three preconditions **(now five — see the correction at the end of this cell)**, and the scenario drives one CONSTRUCTED arm down each branch — a forced crossing with nothing committed after the checkpoint (every index must hydrate, nothing backfilled) and a crossing followed by one write to the indexed properties (every index must be rebuilt, and that write must be reachable THROUGH the rebuilt indexes, verified against an independent full scan with the index-serving leaf operator asserted). Both arms assert the engine-scoped population counter in BOTH directions, because a hydrated index and a rebuilt one are by contract indistinguishable in their answers; the arms deliberately do not count towards the run's checkpoint or crash statistics, so the #2464 gates keep measuring the IN-LOOP cadence. It also carries the intersect-planner probes (rmp #2490): a two-predicate conjunction over the two BTREE-indexed properties — the only shape that reaches the planner's bitmap composition and its budgeted `RangeCount` / `RangeCountFrom` gate — in an unbounded-above and a bounded string spelling, each result-verified against an independent full-scan reference, required to compose in the physical plan, required to retain its mandatory residual `Filter`, and (for the unbounded arm, whose two conjunct counts are both exact) required to AND its parts in the order the client-side cardinalities predict — the only observable a budgeted count's VALUE has. A single-property control must seek through the SAME operator WITHOUT composing, so the composed marker cannot be mistaken for "some index was used", and a terminal gate requires a composition, a non-empty composed result, and a solo seek to have been observed. **CORRECTED (rmp #2797, commit `806fbdb0`; verified 2026-09-08 at `efd32fb9`) — hydration now turns on FIVE preconditions, not three, and the new one changes what an operator should expect.** Recovery refuses the WHOLE IMAGE on any of three grounds, in this attribution order (`indexImageReason` in `store/recovery/index_payloads.go`): the snapshot was not self-sufficient (no `mapper.bin`, so the payload's raw node ids name nothing); the manifest carries no `indexes_commit_ts`, so the instant the payloads describe is unknown; or the manifest's `index_builder_epoch` is not `snapshot.CurrentIndexBuilderEpoch`. It then refuses a payload individually when the `indexes/<name>.bin` file is missing or fails the CRC32C the manifest recorded (`ErrIndexPayloadUnreadable`). The fifth is the caller's, because only the engine knows which `(label, property)` an index name covers: `Result.WALSuffixTouchesNodeIndex`, checked first at `cypher/index_hydration.go:203`. **The epoch check is the operationally significant addition:** rmp #2778 and rmp #2792 fixed two backfills that had indexed values an open transaction wrote eagerly and then rolled back, and such a payload is DURABLE — hydrating it would reinstate the fabricated entry, so no fix to the builder could ever reach a store already on disk. Refusing every epoch but this build's own is what makes those fixes reach the disk, and the comparison is EQUALITY, not "at least": absent (every snapshot written before the field existed) and newer (a builder this build knows nothing about) are both refused. The consequence for a consumer is that **every store written by an earlier build rebuilds its secondary indexes on first open instead of hydrating them** — once, correctly, from the recovered graph, and visible in the population counters and in `store.recovery.indexes.*`. |
...
| `search` | deterministic | The `search/` algorithm battery over the live graph: structural parity against the oracle, algorithm correctness against independent references, and the 58-row context-cancellation family. |
...
| `search-crash` | deterministic | The same three families, validated on the crash + recovery-survived graph. |
...
| `checkpoint-teardown` | concurrent | ST3. A background `checkpoint.Checkpointer` against a full-stack durable store while concurrent writers commit over the Bolt wire, then teardown through the crash-safe `store.DB` shutdown order (final checkpoint → stop checkpointer → close WAL under the commit-lock quiesce), racing the close against a fleet of in-flight commits. Asserts recovered ⊇ acked with no phantom, that the checkpointer goroutine joined, and that no `wal.ErrWriterClosed` ever surfaced as an acked commit. |
| `readtx-isolation` | concurrent | ST7. Read-only transactions (`Engine.BeginReadTx`) counting nodes while a writer commits atomic batches: no read may ever observe a partial transaction, and since rmp #2307 the stronger **snapshot** isolation is asserted too — a read observes one fixed instant for the transaction's whole lifetime. Half the readers are abruptly cancelled mid-run; the store is then crashed and reopened, and recovery must restore a whole number of committed batches, never a torn one. |
| `wal-corruption-failstop` | deterministic | ST5. Many single-node commits to a WAL-only store, closed cleanly (no torn tail), then one byte flipped inside an already-durable INTERIOR frame via `SimDisk.CorruptRange` so that frame fails its CRC. Asserts the recovery corruption gate: replay reports genuine corruption rather than a benign torn tail, recovers exactly the committed prefix before the bad frame, and the store fail-stops rather than appending onto the corruption. |
| `checkpoint-dirfsync-fault` | deterministic | ST6. A one-shot fault armed on the WAL's post-rename parent-directory fsync, then a checkpoint: the snapshot publishes, the WAL-prefix truncation renames the suffix-only WAL, and the parent-dir fsync faults. Asserts the `wal.Writer` poison-after-rename contract — the checkpoint errors and every later `Append`/`Sync` returns the sticky error — YET a reopen through real recovery reconstructs the exact committed state. |
| `io-roundtrip-fault` | deterministic | ST8. A deterministic directed graph round-tripped through the `graph/io` CSV and JSONL edge-list formats over a `SimDisk`: a clean export/import that must reproduce the graph exactly, then an export under a tight ENOSPC bound that must fail cleanly with a typed error and leave no partial artefact a re-import silently accepts as the graph. GraphML, which carries labels and properties rather than a weighted edge list, gets its own property-graph round-trip beside them. |
| `bulk-load-oracle` (`internal/sim/bulk_load_oracle.go`) | deterministic | **The offline bulk LOADER's full contract, against an independent model** (rmp #2488). `bulk-vs-online` above drives `store/bulk` for concurrency and resource stability, but compares ONE number — the row count — against the constant it generated the rows from, discards the returned CSR, publishes to a real OS tempdir no fault can reach, and only ever configures Directed+Multigraph. This scenario occupies the content and fault gap beside it, and every arm adjudicates the LOADED GRAPH against `bulkOracleModel`, which reimplements the documented ingest rules in plain Go and calls neither `graph/adjlist` nor `csr.OrderRuns` — so a defect the engine's two builders SHARE is still visible, which a differential between them cannot be. Arms: `Drain` clean and ctx-cancelled mid-stream (the cancel point exact by construction, the producer sending into an unbuffered channel); `AddBatch`, including the PARTIAL ingest its godoc promises before `ErrTooManyRows`, asserted as the surviving row count *and* the content of the accepted prefix; the `MaxRows` crossing on all three ingest entry points; all four Directed × Multigraph configurations built twice and required to emit a **byte-identical** csrfile and identical CSR slices under `Parallel` true and false; publication onto a `SimDisk` under armed sync, rename, ENOSPC and parent-fsync faults; a media-fault disk; a corruption arm; and a **host**-crash differential across the publish rename → parent-fsync window, pinned to both of its legal branches rather than sampled (a `CrashProcess` discards nothing, so only `CrashHost` can revoke a dirent). The post-fault oracle is deliberately **three-way** — absent, complete, or REJECTED — because a faulted sector lets the writer return nil over an image the reader will refuse, and a two-way oracle would have reported six false defects at the catalogue seed. **Two limits are stated rather than glossed:** the goroutine fan-out is unreachable from outside package `bulk` (`Finalise` matches `csrDirectEligible()` first and no public `Options` can falsify it), so the byte-identity arm certifies `buildCSRDirect` — the code a `Parallel: true` caller actually runs — and nothing about multi-goroutine determinism, guarded by a tripwire test that fails if a shard-capacity knob is ever added; and `Finalise`'s own publication binds the OS backend, so the fault arms drive the identical writer core through `csrfile.WriteToFileWith`. See [The bulk loader's full contract](dst-feature-coverage.md#the-bulk-loaders-full-contract-content-streaming-caps-and-a-faulted-publish-rmp-2488). |
...
| `bolt-cert-rotation` (`internal/sim/bolt_cert_rotation.go`) | deterministic | **TLS certificate rotation under fault, adjudicated by a real handshake** (rmp #2481). `server.CertReloader` is the operator seam for rotating the Bolt server's TLS material without a restart, and its whole promise is a negative one: a rotation that goes wrong must leave the LIVE certificate untouched. A promise of that shape is only tested by breaking the rotation. Eleven steps run in sequence — an initial load, a clean rotation, then a TORN key (only a seed-chosen PREFIX of the PEM landed), a GARBLED key (all bytes landed, then a sector under them went bad, injected through `SimDisk.CorruptRange`), an ABSENT key (unlinked and never rewritten), a MISMATCHED pair (the new cert beside the old key — the dangerous case, because BOTH files parse), the background poller reporting a failure, the completed rotation, a rotation that deliberately does NOT advance the file mtimes (rmp #2558), and finally two pairs that parse and pair PERFECTLY and still cannot serve — an EXPIRED leaf and a NOT-YET-VALID one (rmp #2557). Each verdict has two halves, and it matters which answers which question: WHICH pair is live is settled by the served leaf's Common Name, while THAT the served pair is usable is settled by completing a genuine TLS 1.3 handshake through `crypto/tls` over a LOOPBACK TCP pair against whatever `GetCertificate` returns, with the client trusting exactly that certificate. `crypto/tls` is the independent reference for the second half — a successful handshake proves the certificate parses, the name matches, and the private key genuinely corresponds to the certificate's public key, which is what a mismatched rotation destroys and a byte comparison cannot see. It cannot by itself detect that the WRONG pair went live, which is the CN half's job; crediting the handshake with that check would overstate it. Measured: `read key: open …: no such file or directory` for the absent arm and `tls: private key does not match public key` for the mismatched one, with `rotation-B` still in service and still handshaking through all four faults, and `rotation-C` taking over the moment its key lands. Two documented-but-untested contract halves are pinned with it: an unloaded `CertReloader` REFUSES to serve rather than handing the TLS stack a nil pair, and `NewCertReloader` over a torn key FAILS CLOSED, because the initial load is mandatory. The fixtures are Ed25519 with FIXED validity bounds precisely so the PEM bytes are a pure function of the seed — an ECDSA pair, whose signature draws randomness, would not be reproducible — and the projected files' mtimes are stamped EXPLICITLY so that the timestamp is a deterministic property of the run — which until rmp #2558 was load-bearing because `Reload` short-circuited when neither mtime advanced, and is now what makes the OPPOSITE arm expressible. `SimDisk` is the image authority for every version of every file and every fault applied to it; the bytes are then projected onto a real temporary directory because `CertReloader` reads through `os` and `tls.LoadX509KeyPair` and exposes no filesystem seam (the precedent is `wal_writer_surface.go`). The torn prefix is produced by an ACTUAL host crash: the prefix is written and `Sync`'d, the remainder is written and left un-synced, and `SimDisk.CrashHost` reverts the file to its durable image — which since rmp #2535 advances only on a `Sync` that returned nil, so the crash discards exactly the un-synced remainder (measured: 85 of 119 bytes survive, and the arm fails loudly if a crash discards nothing). An earlier draft of this row asserted the reverse — that a crash never discards un-synced data — which was the pre-#2535 model; it was corrected against `internal/sim/disk.go` rather than trusting the note. Because the torn and garbled arms produce the IDENTICAL parse error, the non-vacuity gate compares the key sizes each step left on disk — a torn key must be SHORTER than the key it truncates (measured 85 of 119 bytes), a garbled one exactly as long (119 of 119), an absent one zero — so the roster cannot name two faults where only one was applied. It also requires at least one reload to have succeeded, at least one to have failed, and the certificate in service to have genuinely CHANGED, since a reloader that ignored every rotation would pass every retention clause. The handshake oracle's own falsifiability is proved by pointing it at a certificate issued for a different name, which `crypto/tls` must reject. Two additions from rmp #2557. The scenario now installs material whose ONLY defect is its validity window — a leaf already past its `NotAfter`, then one whose `NotBefore` has not arrived — because that is the commonest real rotation incident (a renewal that produced expired material, or a clock skew on the renewing host) and it defeats every parse-based check: before the fix `Reload` validated only the PAIRING and swapped the doomed leaf into service, evicting a certificate that still worked. The refusal is adjudicated by CAUSE and not merely by failure: the step records `errors.Is(err, server.ErrCertOutsideValidity)` at the point the error is returned, and the oracle fails in BOTH directions — a validity arm refused for some other reason, and a parse fault reported as a validity refusal — because a shared failure signal cannot attribute a refusal. The reloader's own validity clock is pinned to the same instant `tls.Config.Time` uses, since a leaf's window is now read by the ENGINE as well as by `crypto/tls` and an unpinned engine would make the fixed fixture bounds a time bomb on both sides. The other addition is a fix to the harness: the verifying handshake ran over a `net.Pipe`, which is synchronous and unbuffered, so a FAILING handshake deadlocked it — the server still flushing its flight while the client flushed its alert, neither reading. Measured with #2557's refusal disabled, the run hung to the 40s test timeout with both peers parked in `crypto/tls.(*Conn).flush`, which means the handshake oracle could not report the single condition it exists to report. Over a loopback socket the kernel buffer absorbs the flight and the same state is now reported in 0.03s as nine attributed violations. One further arm, from rmp #2558: a rotation whose material is perfect and whose ONLY unusual property is that it left the file mtimes untouched — a rename from another directory, `cp -p`, a restore from an archive, or two rotations inside one filesystem timestamp tick. `Reload` short-circuited on "neither mtime is After the last successful load" and reported SUCCESS having loaded nothing, so a rotation performed to REVOKE a certificate could be silently ignored while the operator believed the material had been replaced, with `onError` never firing because the call succeeded. Measured against the pre-fix code the arm reports exactly that shape — `preserved-mtime-rotation reload-ok serving=rotation-C` where `rotation-preserved` was expected. Two structural guards keep the arm honest, because its fault is a TIMESTAMP and nothing else: the mtimes it leaves may not exceed the previous step's, and it must FOLLOW a step whose reload succeeded — placed after a refused step the same rotation would carry a timestamp newer than the reloader's bookkeeping and even the defective code would have loaded it, so the arm would pass everywhere and prove nothing. The skip is now keyed on a SHA-256 digest of each file's bytes, which is why the arm passes today. |
| `bolt-tx-registry` (`internal/sim/bolt_tx_registry.go`, `bolt_tx_terminate.go`) | deterministic | **The Bolt transaction registry, its idle reaper and operator termination, on a fake clock** (rmp #2482). `Server.Transactions`, `Server.TerminateTransaction`, `Options.MaxTxIdleTime` and `Options.MaxOpenTxPerPrincipal` were added after the round-3 audit's whole-server stall and the DST never touched any of them. The scenario runs two arms, each on its OWN `SimServer` so their timer counts stay attributable, both driven through `NewSimServerTxRegistry`: the server's clock is an injected counting `clock.Fake`, while the in-memory listener keeps REAL time, because sharing one fake would make every parked connection register a timer of its own and destroy the only barrier the arms have. **Arm 1, abandoned-registry:** five connections BEGIN one advance apart under their own principals and go silent, and the listing is adjudicated FIELD BY FIELD against the harness's own plan. This is what the DST adds over `bolt/server/tx_introspection_test.go`, which runs on the wall clock and can assert no more than `Elapsed > 0`: here `StartedAt` is EXACTLY the fake instant the harness opened at and `Elapsed` is exact to the nanosecond, because the harness owns every advance and the arming barrier guarantees none is in flight while a transaction registers. The barrier itself had to be built rather than assumed — `syncTxTimer` arms the reaper AFTER the response loop has flushed BEGIN's SUCCESS (`serve.go:1350` against the flush at `:1501-1505`), so neither "the client got SUCCESS" nor "`Transactions()` lists N" proves the reaper is armed, and an arm that advanced on either would clamp `Until` to zero and land the reap an advance late. A `NewTimer`-counting clock decorator closes the gap, and it is attributable only because `syncTxTimer`'s is the ONLY `clock.Clock` timer registration in `bolt/server` (verified by exhaustive grep; `tls_reload.go` uses `time.NewTicker` from the standard library, so it cannot reach the probe). An independent `idleReapModel` predicts the reap ordinal for each transaction from the plan's arithmetic alone and consults nothing from `bolt/server`; measured `[6 7 8 9 10]` predicted and observed, with five quiet ordinals in front at which the reaper must DECLINE to reap. Each reap is attributed by the typed FAILURE only `Session.reapTimedOutTx` arms, so an absent entry — an ambiguous post-state a rollback or a dropped connection would produce identically — is never taken as proof on its own. Four autocommit writes straddle the reap as WAL witnesses, and the per-advance bracket measures 0 frames and 0 bytes charged to the reap; the bracket is a live instrument, MEASURED by moving one window inside it and watching it read 4 frames / 195 bytes. **Arm 2, operator-terminate:** its own server with both bounds at ten minutes of fake time and ZERO advances, so `clock.Fake` can deliver to no waiter at all and every departure from the registry is provably the operator call's doing — asserted as a non-vacuity clause, not argued. It ends a live id (which leaves, while the other two do NOT), refuses two STALE ids the harness watched go live and then finish (one terminated, one committed — both strictly stronger than the never-seen id the pre-existing test covers), and pins **successor immunity**: the victim's connection RESETs and opens a second transaction, whose id suffix the harness predicts from the BEGIN ordinal, and the predecessor's id is still refused while the successor stays listed across a settle window. The terminated transaction's staged node is absent from the live engine AND from a graph reopened through real recovery after a crash, with the WAL bracket across the termination at 0 frames and 0 bytes. This arm compares the listing as a SET rather than a list, and that is a finding: with zero advances every entry shares one `StartedAt`, and `txRegistry.list`'s insertion sort swaps only on a strict `Before`, so equal keys are left in Go map-iteration order — asserting an ORDER there would be asserting the map. The client's reply is recorded verbatim and pinned against named constants. It was pinned against constants whose text was FALSE on both halves for an operator termination, so that the eventual correction would fail the arm deliberately — and it did: rmp #2560 gave the operator path its own reason (`Neo.ClientError.Transaction.Terminated` / "the transaction has been terminated by an operator request"), the constants split into a deadline pair and a terminate pair, and this arm now adjudicates against the latter, with a named falsifiability case for the specific regression of borrowing the deadline reason back. The live control aims the same termination at the BYSTANDER instead, changing nothing else, and four independent clauses fire. |
...
| `bolt-decode-pressure` (`internal/sim/bolt_decode_pressure.go`) | deterministic | **The engine-wide inbound-decode memory pool, adjudicated against a closed-form model of its own arithmetic** (rmp #2487). The harness had exactly one arm near inbound-memory abuse — the `BoltAbuser`'s oversized frame, which drives the PER-MESSAGE framing cap on ONE connection. The pool that bounds every connection at once (`packstream.InboundBudget`, created once per server at `bolt/server/serve.go:654`) had never been touched, and neither had either nesting cap. **The load-bearing oracle is a closed-form model, not the server's word**: the harness re-derives what a RUN's decode holds from the shared pool out of packstream's published per-slot costs — `held = 1344 + len(query) + len(key) + 48n`, where 1344 is the RUN struct (32 + 3x48), the one-entry parameters map (512 + 112), the parameter list's container (32) and the empty extra map (512), and the two string terms are there because `ReadString` charges its payload 1:1 against the same pool (`decoder.go:712`), which is not obvious and was found by measurement. The model was calibrated by binary search on the smallest budget that admits a payload — `48n + 1353` for every n tried with an 8-byte query and a one-byte key, exactly as the closed form says — and it names the boundary EXACTLY: at a 4 MiB ceiling n=87353 (slack +0 B) is SERVED and n=87354 (slack -48 B) is REFUSED. **A one-element-wide boundary is the strongest available refutation of "the per-message cap did it"**: the two messages differ by 48 charged bytes out of ~4 MiB, both are ~200x under the 16 MiB framing cap and 32x under the 128 MiB per-message decoded-collection cap, and the CONTROL — a second server differing in the ceiling and in nothing else — SERVES the identical 165,796-element bytes the pressured one refused, writing its node into its own engine while the pressured engine holds none live and none after real WAL recovery. **The sharpest clause is that three abuse vectors draw three DIFFERENT answers**, each measured rather than inferred: the aggregate pool answers `Neo.TransientError.General.OutOfMemoryError` and leaves the session READY with no RESET; the wire nesting cap (`packstream maxValueDepth = 128`) answers `Neo.ClientError.Request.Invalid` / `malformed Bolt message` and also leaves it READY, because both are refused ABOVE the session state machine (`serve.go:1258` and `:1289`); and a SECOND, LOWER, INDEPENDENT cap this task discovered by measurement — `cypher maxParamBindDepth = 32` (`cypher/api.go`) — answers `Neo.ClientError.Statement.ArgumentError` and leaves the session FAILED, because it travels through the state machine into `BindParams`. That third answer was `Neo.DatabaseError.General.UnknownError` when this scenario measured it, and reporting that is what got it fixed: rmp #2570 reclassified a wholly client-supplied payload as the client fault it was, in the `Statement` family because the message decoded and the statement was dispatched — what is invalid is the ARGUMENT, not the form of the request. These arms pinned the old answer, so the correction failed them on purpose and they were updated with the ticket. The session states still differ for a reason that is not an inconsistency: staying READY is reserved for back-pressure, where retrying can succeed, and a depth refusal is deterministic. A server that collapsed any two would be indistinguishable, from a client's side, from one with no pool and no depth cap. The classification segment is asserted on its own, read out of the OBSERVED code: neo4j-go-driver's `IsRetriableTransient` tests `classification == "TransientError"` (`bolt/server/errors.go:130-131`), so "typed RETRYABLE backpressure" is a checked property and not a restatement of the literal. **The nesting family is bracketed at every boundary and is tiny on the wire**: 32 accepted / 33 refused and 127 refused-by-the-engine / 128 refused-by-the-decoder, identically for LIST chains and MAP chains, at 55 to 4046 wire bytes at the catalogue seed and at most 6166 on any seed (the deliberately excessive arm's chain length is drawn from `[2048, 6144)`) — all under a 64 KiB anti-confound ceiling, because a payload refused for its SIZE proves nothing about a DEPTH cap. The chains are hand-built from the marker table and not through `packstream.Encoder`, which CANNOT express them: `writeValue` carries the same depth bound as `readValue`, so the encoder refuses to encode the abuse its own decoder must reject. The pre-authentication arm is the one that isolates the wire cap cleanly (no parameter is bound, so the engine's cap is not in the way): a 127-deep HELLO succeeds, a 128-deep one is refused, and the connection survives with the session still UNauthenticated — a following plain HELLO succeeds, which it could only do from NEGOTIATION. **No-leak is proved through the wire, not by reading the pool.** `InboundBudget` exposes `Enabled`/`TryReserve`/`Release` but no `Remaining`, and the server's pool is unexported; instead the run repeats the calibrated boundary message after every abuse arm, and its acceptance at slack +0 B means a leak of a single byte would have been caught. 22 deterministic contract clauses and 9 `nv-` ones. |
| `bolt-decode-swarm` (`internal/sim/bolt_decode_pressure.go`) | concurrent | **The aggregate cross-connection vector: K connections pushing large-collection parameters at one shared pool while an honest client works** (rmp #2487). It is registered separately because it is not bit-reproducible AND because the aggregate vector is only reachable concurrently: every charge is released before its reply is written — the reassembly reader releases on every return path from `ReadMessage` (`bolt/proto/chunking.go:160-165`) and the decoder's hold by the deferred `ReleaseInboundBudget` (`serve.go:1419-1423`) — so a single-threaded script can never observe two charges outstanding, whatever it sends. Each abusive message is sized at 55% of the pool by the same model, so ONE fits and TWO cannot. **Two oracles about the honest client had to be constructed rather than raced for, and the cost of not doing so was measured.** With both sides started together and each running a fixed count, the abusers finished in ~38 ms while the honest client was still pausing, and exactly ONE of 24 honest exchanges straddled a refusal — a coverage clause resting on a single sample. The window's ENDS are now pinned (the honest client waits for the first refusal; the abusers push until it finishes), which took it to 9 of 24; that is still a race, and the measurement says why: under `-race` a narrow honest exchange takes 633 us at the median (357 us to 902 us over 20 samples) while refusals arrive about once per 3.8 ms of honest in-flight time, so most land in a gap. So the WIDTH is controlled too: every sixth honest exchange holds its open stream for 50 ms between RUN and PULL — genuinely in flight, the server holding a cursor — which is ~13x the inter-refusal interval and ~79x the median narrow exchange, and the clause gates on those alone. Measured 4 of 4 wide exchanges straddling across 25 seeds under `-race`, with the NARROWEST wide window holding 9 to 11 refusals against the 2 to 3 a 20 ms hold gave; the 100-seed soak sweep, which runs under heavier concurrent load, saw a worst case of 6 and asserts that worst case rather than only the pass. The hold is deliberately NOT a wait for a refusal to be counted: that would make the clause true by construction of the HARNESS instead of by behaviour of the SERVER. An independent density clause requires the whole honest run to contain at least 8 refusals (measured 41 to 47). **The liveness bound is a wall clock, and that is the legitimate case rmp #2567 left standing**: a server that starved honest traffic under aggregate pressure would never serve it at all, so the wait is unbounded and only a clock can see it — the bound is set against a MEASUREMENT rather than a guess — 30 s against a measured 633 us median and 902 us worst honest exchange with the fleet pushing under `-race`, about 33,000x the worst observed service time — the message that fires says STARVED, and it is paired with the claim that actually matters, which involves no clock at all: every honest exchange must return the value the harness chose for it. The sizing also has to keep the honest client clear of the REASSEMBLY reader, whose budget breach is not the connection-preserving refusal the decode layer's is — it returns a READ error (`chunking.go:223-227`) and the serve loop tears the connection down (`serve.go:1237-1247`) — so the pool floor is computed (one abuser's charge is 4.4 MiB of an 8 MiB pool, leaving 3.6 MiB; three refused abusers transiently holding a 1 MiB reservation chunk each leaves ~0.6 MiB against ~100 B of honest reassembly) and `swarm-no-transport-loss` reports it by name if the sizing ever stops holding. Measured on one run at the catalogue seed under `-race` (the counts are scheduler-dependent and are a sample, not a constant): 13 served, 63 refused, 24/24 honest served correctly, 4/4 wide exchanges straddling with the narrowest holding 11 refusals, 59 refusals across the honest run, 4/4 abuser connections still serving afterwards, and the full-ceiling probe admitted at +16 B of slack. **That probe is sized to a MEASURED boundary, not a modelled one (rmp #2579).** A seven-probe scan runs against the pristine pool before a single fleet goroutine starts — measuring during the swarm would read whatever the other connections happen to be holding, so it is deliberately done before they exist — and the post-swarm leak probe is sized to the largest element count the server actually admitted. That is what makes `nv-swarm-leak-probe-tight` a statement about the server rather than the harness guard it used to be, demonstrated by shrinking the real pool 48 B and watching the measured boundary drop one element and the slack widen from 16 B to 64 B. `nv-swarm-window-spans-boundary` reports a scan that failed to bracket, so the fallback to the modelled boundary can never be taken silently. 7 contract clauses and 8 `nv-` ones. |
| `generation-swap` (`internal/sim/generation_swap.go`) | concurrent | **The lock-free CSR publisher's refcount lifecycle** (rmp #2491). `graph/generation` had ZERO simulation coverage, and its failure modes — a leaked refcount, a generation recycled while a reader still holds it, a drain that never completes, a `Close` that wedges — are all INVISIBLE in the answer a reader gets: a reader handed the wrong generation still returns a well-formed graph, just not the right one. The oracle is therefore an IDENTITY oracle. Every generation the seeded plan builds is a different graph that carries its own publish sequence number inside its content (node 0 has exactly one out-neighbour, `1+seq`), so a reader decodes from the artefact alone which generation it believes it holds and then compares the WHOLE traversal against the model's independently computed fingerprint for that sequence — with a third channel, the generation POINTER's recorded sequence, cross-checked terminally. The package's own `csr_rotation_consistency_test.go` cannot reach any of this: its fixture builds a one-edge graph for every seed, so its "Size() must equal 1" oracle holds for every generation that ever existed and discriminates none of them. Each refcount clause is either a STRUCTURAL BOUND (a reader inside its access window holds one reference, so the count can never be below 1; a reader holds at most one outstanding increment and the publisher at most one hostage, so it can never exceed readers+1) or is taken AT REST, after every reader goroutine has been joined and the publisher stopped — the only point at which "every generation's refcount is zero" is an exact statement rather than a sample of a moving target, and the sound home for the task's refcount-returns-to-zero requirement. The drain-timeout arm is STRUCTURAL, not thresholded: the publisher itself holds a reference on the generation it is about to supersede for the whole `PublishWithDrain` call, so the wait condition `prev.refcount > 0` is permanently true and the timeout branch is the only reachable exit at ANY positive timeout — verified at 1ns, 1µs, 1ms and 20ms — while the plan forces the very next publish to be an unbounded drain that MUST return nil, so timeout-when-held and drain-when-free are both measured and neither passes vacuously. A second arm calls `Close` with the reader fleet live and a publisher racing it. Reproducibility is stated exactly rather than claimed: the whole plan and every per-goroutine sub-seed are drawn up front on one goroutine (the `RunConcurrent` precedent), so the plan — generation count, every generation's adjacency/shape/fingerprint, the publish op per sequence, the drain-timeout index, and therefore the plan digest the report pins — is a pure function of the seed and is invariant across reader counts (the sub-seeds are drawn LAST for exactly that reason), while the acquisition counts and which generation each landed on are NOT and are reported as telemetry, never asserted against a threshold. Non-vacuity is likewise structural: each reader's first acquisition is barriered before the publisher may publish (so it must observe sequence 0) and its last is taken after the publisher finished (so it must observe the final sequence), which makes "every reader straddled a swap" a fact of the construction. USE-AFTER-FREE is NOT reachable and is not claimed — Go's GC keeps the CSR alive while a reader holds the pointer, so detecting it would need a poisoned allocator; what the sentinel clauses cover is USE-AFTER-RECYCLE. The wide 64/256/1024 fleets and the 64-seed geometry sweep are SHORT layer, not soak (`generation_swap_wide_test.go`, measured 0.46 s under `-race` on 10 cores), so the published concurrency levels are exercised on every run of the gate. See the clause-by-clause mutation table in the file header, which carries the reproduced-versus-inherited split for each mutation |
| `fluent-query` (`internal/sim/fluent_query.go`) | deterministic | **The fluent pattern-query engine as a SECOND, independent read path over the same graph** (rmp #2492). `graph/query` had ZERO simulation coverage — nothing under `internal/sim` imported it, and its only exercisers were two examples and its own in-package tests — yet it is a full reader of the same `lpg.Graph` the Cypher engine reads, with its own working-set representation (a roaring64 NodeID bitmap), its own label seeding, its own tombstone pruning and its own index-seek decision logic, none of which is shared with the Cypher planner. The epistemology is deliberately NOT the one `differential.go` uses. That facility compares the engine with ITSELF (default planner vs the same planner with the hash join disabled), which is sound there because the engine GUARANTEES the two variants are result-equivalent, so any divergence is a regression and it does not matter which side is right. Here the two sides are INDEPENDENT implementations with no such guarantee, so "they agree" is a weak claim (both read the same label index and the same property store, and can be wrong the same way) and "they disagree" would name no culprit. The arbiter is therefore the MODEL — `GraphOracle`, computed from the op stream alone — and every probe carries three SEPARABLE clauses (`fluent-vs-oracle`, `cypher-vs-oracle`, `fluent-vs-cypher`) so a red run says which path is wrong; a defect in the shared substrate moves both engines together and shows up as two red oracle clauses with a SILENT fluent-vs-cypher clause, which is the correct attribution. A fourth channel, neither engine, is a SUBSTRATE view built by walking the Mapper directly and reading each survivor's `name`; its live-name set is held to the model before any probe runs, so a downstream failure cannot be blamed on a substrate that had already diverged. `query.New` takes both a graph and a CSR, so the scenario builds a FRESH pair at every probe point — `BuildFromAdjListLive` and the tombstone-agnostic `BuildFromAdjList` — and requires `Out()` to answer IDENTICALLY over both; that is a theorem of the two prunes acting together (a tombstoned SOURCE is dropped by the seed step, a tombstoned TARGET by `Out`'s own prune, which between them remove exactly the arcs the live filter removes), so it is a detector rather than a tautology, and the CSR is read by nothing except `Out()` so the label/property/range probes are provably CSR-independent. The tombstone attack is gated on the DETERMINISTIC seed path: `seedAllLive` walks the Mapper, which never forgets a slot, so every tombstoned id is yielded on every call for the whole run — whereas the label-bitmap corpse count is swept by lpg's BACKGROUND VACUUM (`applyDeferredIndexRemovals`) and MEASURED at 3 and 2 for the same seed in the same process, so it is recorded as telemetry and gated on nothing. `Out()`'s ghost-arc prune branch is UNREACHABLE on the live graph (MEASURED: `DETACH DELETE` strips the deleted node's arcs, so the raw and live CSR builds report the same `Size()` on all 24 sweep seeds), so it is driven by a constructed side fixture that uses `lpg.RemoveNode` to tombstone without stripping arcs and ASSERTS its own precondition before asserting the answer. Seek-vs-scan is separated BY CONSTRUCTION rather than by instrumentation: `Vertex(label, pred)` is seek-eligible and `Vertex(label).Vertex(pred)` is not, because `labelsInPreds` of a predicate list with no `WithLabel` is empty and both `trySeekProperty` and `trySeekRange` refuse an empty label list. Served-ness itself is established by ENUMERATING the guard's conditions (manager, kind, `BoundNode`, typed read interface) — stated as such, not dressed up as observation, since the path can only be OBSERVED from inside the package. What is NOT claimed: resurrection (Cypher `CREATE` mints a fresh mapper key, so `AddNode`'s revive path is unreachable), concurrent use of the fluent engine (its contract requires quiescence), and multi-hop chains. The one inconsistency this scenario found — the seek and scan arms disagreeing for mixed INTEGER/FLOAT range bounds — was recorded rather than pinned while its semantics were open, and is now the ASSERTED `range-mixed` clause (rmp #2600). Closing it created a SECOND asymmetry, between two predicates rather than between two arms: the unified range matched a stored `Int64Value(5)` under `Float64Value` bounds while the equality, still requiring a shared kind, did not, so the same data answered differently depending on whether the predicate was written as `WithProperty` or as the degenerate `WithRange`. rmp #2601 unified `equalValue` through the same exact comparator and added the `eq-mixed` clause plus an `equality-vs-degenerate-range` clause that names the asymmetry directly; see the coverage document |
| `typed-schema` (`internal/sim/typed_schema.go`) | deterministic | **The typed schema as a runtime ENFORCEMENT hook** (rmp #2493). Nothing under `internal/sim` had ever called `lpg.Graph.SetValidator`, and nothing imported `graph/lpg/schema`, so every DST run drove a graph with NO validator installed — which made the accept/reject verdict, the all-or-nothing contract of a refused write, and the behaviour of a RECOVERED graph all unfalsifiable under simulation. The task's brief said the hook sits in "the edge-property write paths"; VERIFIED in source it is consulted on **five**, node-side included (`setNodePropertyInfo`, `setEdgePropertyInfo`, `setEdgePropertyByHandleInfo`, `setEdgePropertyAtInfo` and the fused `AddEdgeLabeledWithProperty`), and each is its own arm because the four property stores behind them are genuinely different stores — MEASURED, one write through each path on one pair left `EdgeProperties` showing only the columnar value, `EdgePropertiesByHandle` only the per-handle one and `EdgePropertiesAt` only the per-instance one. The verdict oracle is a DECLARATION TABLE, not the schema: it is the same table `RegisterProperty`/`RequireProperty` are fed, and it never asks `Validate` what the answer should be. Observations are classified by SENTINEL — `ErrTypeMismatch`, `ErrUnknownProperty`, `ErrMissingRequired` are three different refusals and an arm that accepted any error would pass while the wrong one was raised. Coverage is CONSTRUCTED, not drawn: the five paths x three verdicts are fifteen cells and the battery SWEEPS every cell once per epoch in a seed-shuffled order, so the seed decides the order and never the coverage. Every refusal is followed by a five-clause no-mutation battery — the value through the path's own accessor, the value through the columnar store's SECOND accessor, the node and edge population, whether the property key was INTERNED (MEASURED: it is not, the hook runs before the intern), and on the fused path that neither the edge nor its fresh endpoint appeared. `Graph.ValidateNode` gets its own arm because it **had no caller anywhere outside `graph/lpg`** before this task (MEASURED: only the package's own internal `NodeValidator` dispatch and its own tests; every `graph/lpg/schema` hit is `Schema.ValidateNode`, a different receiver): required-property existence is embedder-invoked, never engine-invoked, so the scenario invokes it at the node-finalisation boundary itself and pins the mid-build rejection beside the finalised acceptance, plus an unlabelled control, the never-interned benign exit, and a fixture whose forbidden value was written BEFORE installation — the only route, short of the recovery bypass, to ValidateNode's kind re-check over already-present properties. The recovery asymmetry is pinned as five clauses from one constructed probe: a write the live validator refused is ACCEPTED on the freshly recovered graph (the schema is not a snapshot component, so nothing re-installs it), ValidateNode reports clean there too, the identical write is refused once a fresh schema is bound to the RECOVERED registries, ValidateNode then reports the planted value as a type mismatch, and reports clean once it is repaired. **The scenario surfaced a real asymmetry between the two durable paths.** The Cypher engine validates BEFORE the WAL (`walMutatorAdapter.SetNodeProperty` performs the validated write, then buffers the op), so a refused value never reaches the log — MEASURED across a crash. `txn.Tx.Commit` fsyncs every buffered op and only then applies them, so a refusal there returns `txn.ErrCommittedNotApplied` with the frame already durable, and the validator-less replay RESURRECTS it: MEASURED, `age` committed as a STRING under a schema declaring INTEGER was absent from the live graph and present, still a STRING, after a host crash and reopen. That is pinned in both directions — the accepted sibling value must survive or the arm is vacuous — as a measurement, not an endorsement. The direct-API arms run on a graph the scenario OWNS rather than the store's, because a direct `lpg` write does not reach the WAL and would put nodes in the engine the `GraphOracle` does not model, breaking count parity and letting a checkpoint make them durable. Fixing a panic this scenario found in `graph/lpg` was a prerequisite for it running at all — see the coverage document |
| `count-store` (`internal/sim/count_store.go`) | deterministic | **The derived relationship count-store adjudicated cell by cell against the shadow model, and the reopen-time `RecomputeReset` asserted CORRECT rather than merely executed** (rmp #2494). `count.Store.Snapshot` had NO production caller and no `CountE`/`CountT`/`TDirty` query shape was in the repertoire, so although the reopen recompute RUNS on every recovery already (`OpenSimStore` builds a fresh engine per reopen and `NewEngineWithOptions` calls `recomputeCountStore` at construction) nothing ever compared its result to anything — the exact fail-silent class the DST exists for, since a wrong count store raises no error and returns correct rows from a plausible-but-wrong plan. `cypher`'s own `diffCounts` and rapid property test cannot close it: they compare the store with the GRAPH, which is what `recomputeCountStore` does, so they structurally cannot witness the recompute. One accessor was added, `Engine.CountSnapshot`, returning a COPY of the cells and dirty markings rather than the `*count.Store` (a handle would hand callers `Apply`, `MarkDirty` and `RecomputeReset` over the engine's own derived state). The model is `GraphOracle.countStoreModel`, recomputed from the op stream alone and keyed by NAME: interned ids MEASURED as stable across both a WAL-only replay and a snapshot+WAL reopen — `Vip` kept id 2 even with no live carrier — but nothing documents that, so ids are resolved through the recovered graph's own registry at comparison time. The LIVE phase skips a dirty-covered cell, because the maintenance path deliberately marks the IN families non-exact instead of writing a wrong exact; the RECOVERED phase skips NOTHING and requires all four dirty sets EMPTY, which is the documented self-heal point. How easily dirt is reached was itself a finding: MEASURED, one `DETACH DELETE` of a labelled node dirties `DIn` and `TB` for that label, and the dirty sets only GROW until the next reset, so on any graph that has ever deleted a labelled node the planner's IN-side statistics are vetoed for the rest of the session — which is why the scenario carries a never-relabelled, never-deleted `Hub` label and gates on per-family COMPARED counters, since without it the live `DIn` and `T` clauses would have compared two empty maps. A NEGATIVE cell is constructed on purpose: MEASURED, `SET a:X`, `SET b:X`, `REMOVE a:X` over an `a -> b` edge leaves `T(X,KNOWS,X)` at -1 (the +1 never applied because b had no out-edge; the -1 did, b having acquired X by then), which is legitimate under `Store.add`'s order-insensitivity design and must be dirty-COVERED and GONE after the reopen. The heal is credited only on the TRANSITION — the immediately preceding live observation must itself have been dirty — and the fixture is re-armed after every recovery, because MEASURED before that a 1500-tick run with 19 recoveries healed exactly one negative cell. `Cells()` boundedness is asserted in soak, where MEASURED it peaked at 20 against a ceiling of 25 while the modelled edge count reached 227. **The scenario surfaced a planner correctness defect**: with the anchor swap admissible — which is exactly what a reopen's dirty-clearing unlocks — a single-edge pattern whose SOURCE node is anonymous and labelled returns ZERO rows, reproducible with no store and no recovery, all four spellings rendering the identical EXPLAIN tree and `PROFILE` localising the loss to the `Filter` above the re-rooted `Expand`. Attributed by A/B to `DisableAnchorSwap`; reported rather than fixed, and pinned by a test that fails loudly when the defect is fixed. See the coverage document |
| `pagerank-ranker` (`internal/sim/pagerank_ranker.go`) | deterministic | **The stateful `centrality.PageRanker` across an interleaved serial/parallel sequence** (rmp #2495). `NewPageRanker`/`Run` was not undriven — `search_ctx_cancel.go`'s `PageRanker.Run` row is the one place in that whole table whose identity arm compares two genuinely independent implementations rather than a delegation — but it builds a FRESH ranker per call, Runs ONCE, with default options, on a fixture of a few tens of nodes, which is below `pageRankParallelThreshold` (2048), so no cached state is reused, the reverse-CSR transpose is never built, and the aliasing hazard is structurally unreachable. The in-package tests reach further and still stop short: the bit-identity test runs three times with IDENTICAL options, never touches `GOMAXPROCS` (so which regime the large fixture takes is a property of the host, asserted nowhere) and builds the transpose on the FIRST run if at all; the parallel bit-identity test clamps `GOMAXPROCS` but only for the ONE-SHOT `PageRank`; and the aliasing contract's only mention in the package was a comment claiming a test "mirrors the documented contract" above code that neither copies the result nor Runs again. **A premise in the task was wrong**: `PageRankOptions` has no parallelism knob (VERIFIED — `Damping`, `MaxIterations`, `Tolerance`), and a PageRanker is bound to one immutable CSR, so `live` is fixed and options can never move the regime; the only lever is the process-global `GOMAXPROCS` over a fixture above the threshold, which is also the only way to make the LAZY transpose build land MID-sequence. The regime each Run took is established two ways rather than assumed: a DERIVATION (`live` recomputed from the fixture's own edge list, `GOMAXPROCS` read back inside the window, the documented predicate re-evaluated — labelled as a derivation) and an OBSERVATION that is exact and deterministic (every engine worker starts inside `pprof.Do`, which consults the parent label set through `Context.Value`, so a counting context reads ZERO lookups on the serial path and one per worker at spawn on the parallel one, the spawn provably preceding the first `iterate` return because `iterate` sends on each worker's UNBUFFERED start channel inside the wrapped function). Each worker reads the parent set a SECOND time on exit and `pageRankEngine.close` does not join, so the clause is a BAND — 0 serial, `[workers, 2*workers]` parallel — and MEASURED 4, 8, 9 and 11 at clamps of 4 and 8, the excess being the PREVIOUS window's workers exiting, which is exactly why an equality would have flaked; only band membership enters the digest. Bit-identity is compared on the BIT PATTERN, not with `==` and deliberately not within the existing `pagerankEpsilon` = 1e-4, which is right for the library-versus-reference convergence gap and would make an identity clause unfalsifiable — and is HALF a typical rank on this fixture anyway (MEASURED: median 2.03e-4, smallest 1.69e-4 over 3 069 nodes). The aliasing pin is BOTH directions: the previous window's returned SLICE must read the new run's values (MEASURED: every one of 3 069 elements, at all five transitions) while a COPY must still hash to what it hashed to at copy time, both gated on the two results actually DIFFERING, which is why every window draws its own damping. The whole-sequence shape is AT MOST two backing arrays with at least one repeat, and that was a finding: which buffer a Run returns is `start XOR (iterations mod 2)`, and MEASURED seed `0x8d10afeecdf8dcf` gave all six windows an EVEN count and returned the same array six times, so an "exactly two" assertion was a parity coincidence dressed as an invariant and failed on the first 32-seed sweep. The transpose CACHE is evidence, not a clause — it is observable only through allocation and the counter is process-global — so only the lower bound on the first parallel window is asserted against the floor the structure-only transpose provably needs; MEASURED 117 456 B against a 104 328 B floor, then 11 144 B and 2 288 B for the two parallel windows after it. **A finding recorded rather than fixed**: `PageRank`'s godoc claims its partial L1 deltas are "deterministic across worker counts", which is true for a GIVEN count and not across counts — MEASURED, equal-range partitioned reductions of one pair of consecutive iterate vectors gave five different values (a 73-ULP spread, ~1e-14 relative) for 2, 3, 4, 8 and 10 ranges against the sequential `0x3fb51ef43754e4b2` — so the bit-identity claim holds only because that ~3e-14 relative difference has never straddled the convergence threshold; 17 280 serial-versus-parallel comparisons (40 seeds x 4 dampings x 9 tolerances x 12 worker counts) found ZERO divergences, matching the ~1e-13-per-run estimate, so the `cross-regime` clause is offered as a tripwire for a STRUCTURAL change to the summation order and not as a detector of that coincidence. `GOMAXPROCS` is process-global and handled explicitly: a package mutex spans the clamped phase so two swarm instances cannot decide each other's regimes or corrupt the value through interleaved save/restore, and the value is read back on both sides of every window so a FOREIGN clamp is a harness error, never a false violation. The per-tick half of the same claims, which does run after every crash/recovery, is `pagerankerStatefulViolations` in `search_pagerank.go`. See the coverage document |
| `label-index-scoped` (`internal/sim/label_index_scoped.go`) | deterministic | **The label index's scoped and range surface, driven directly against a naive map model** (rmp #2496). `graph/index/label` is 514 lines and all of it is public; VERIFIED by an `os.walk`+`re` sweep of every production (non-`_test.go`) file (not by `grep`, which on the reference host can return a silent empty result), `NewNodeIndex`, `NewEdgeIndex` and `Scope` have NO production caller and `AddRange`, `RemoveRange`, `Scan`, `Union`, `Serialize` and `Deserialize` have no non-test caller anywhere, so everything lpg does not drive was carried by its godoc alone. **The constructor rationale is established, not assumed, and is sharper than "the scope does not matter here"**: `Scope` is read only inside `Apply`, `Apply` runs only through the `index.Manager` fan-out, and no `label.Index` is ever registered with a manager — `Manager.CreateIndex` is the registry's sole writer and all twelve production call sites register a `hash` or `btree` index, while lpg's two label indexes are maintained by direct `Add`/`Remove` calls. **CORRECTED (rmp #2738, commit `efedd8f8`; verified 2026-09-08 at `efd32fb9`)** — the CONCLUSION survives, both premises no longer do: `graph/index/build.go` is new in this release window and adds a second registry writer and a second `Apply` route. `Manager.indexes` is now written from THREE places — `Manager.CreateIndex` (`graph/index/manager.go:227`), `Manager.DropIndex` (`:238`, which deletes), and the `RegisterFunc` handed to `Manager.FinishBuild` (`graph/index/build.go:401`) — and production reaches the first and third through one switch, `CreateIndexOp.register` (`cypher/exec/create_index.go:282-288`), which uses `CreateIndex` when no build log is attached and `FinishBuild` when one is. `Subscriber.Apply` likewise has THREE call sites: the live fan-out in `Manager.Apply` (`manager.go:309`) and `Manager.ApplyBatch` (`:328`), and the build-log replay inside `FinishBuild` (`build.go:399`), which prefers `ResolvedApplier.ApplyResolved` when the subscriber implements it and the log carries a `BuildResolver`. The enumerated "twelve production call sites" is also stale in both its count and its line numbers: at `efd32fb9` there are EIGHT `Manager.CreateIndex` sites under `cypher/` — `cypher/api.go:1966`, `cypher/index_binding.go:1051`, `:1054`, `:1072`, `:1075`, `:1130`, `cypher/exec/create_constraint.go:125`, and `cypher/exec/create_index.go:284` (passed as a method value) — and every one of them still registers a `hash` or `btree` index, so `NewIndex()` remains correct for both label indexes. `graph/index/label/index.go:17` still asserts the retired "sole writer" claim in its own package documentation. So `NewIndex()` is right for both because the field the other constructors set is never read. The sharper half: `NewEdgeIndex()` would not merely be inert but WRONG if it took effect, because `index.OpAddEdgeLabel` IS constructed (`cypher/api.go:17890`, `:18804`) and IS delivered (`cypher/exec/index_writeback.go:45`) while `index.OpRemoveEdgeLabel` is constructed NOWHERE, so a registered `ScopeEdge` index would take every addition and never a removal. That is now recorded in the package documentation. Overlap and adjacency are CONSTRUCTED, not drawn — the inclusive-to-exclusive `+1` at `nodeset.go:339`/`:378` is where an off-by-one lives and a drawn endpoint pair reaches an exact adjacency only by luck — so thirteen relationships (disjoint, adjacent each side, partial overlap each side, contained, containing, identical, single-element inside and outside, inverted, and the off-by-one empty range) are swept in BOTH directions, twenty-six cells per epoch in shuffled order, each driven once on a fresh label carrying only the base band and once on a label with a long history; the seed decides the order, never the coverage, and a gate names any undriven cell. The model is a plain `map[uint32]map[uint64]bool` that walks the closed interval one id at a time and is ITSELF tested against hand-computed cases; it deliberately does not model when the index deletes a map entry, since that would be a second copy of `nodeset.go` agreeing by construction, so the entry clause is tier-free and one-sided and the excess is reported as a number. **The CRC bounds what the corruption arm can reach**: a raw flip is caught by the checksum wherever it lands — MEASURED at all seven layout regions, and under soak at ALL 143 byte offsets of a small image, every one answered by the CRC and every one leaving a populated receiver unchanged — so the four structural guards are unreachable by corruption alone and a second, RE-STAMPED family is the only way to reach them, each trial naming its guard; guards are classified by whole phrases, since "bitmap" appears in two of them. The re-stamped family also records that a re-stamped `labelCount` of 0 is ACCEPTED and the body's trailing bytes ignored — not a defect against CRC32C, which is not a MAC, but the format ties `labelCount` to nothing. **An over-strong assertion of the harness's own was caught and corrected before it shipped**: the tier arm first compared Add-built against AddRange-built and required byte-identity, which FAILED at width 4 and above because `Serialize` promises determinism "for a given in-memory state", not a function of the logical contents; the claim the module actually makes (inline tier == a bitmap holding the same ids, the #1585 zero-migration claim) HOLDS at six widths with the bitmap side reached by growth-then-trim, and the Add-versus-AddRange difference is kept as a measurement because two indexes answering every query identically can still have different images. The same mistake nearly reached the round-trip clause and produced a finding instead: `image == round-tripped image` is FALSE for a run-encoded label of 4 to 8 ids and the sweep can reach that shape, so what is asserted is that the form is a FIXPOINT after at most one cycle. **Three latent defects pinned to their measured behaviour**, all reported rather than fixed and all firing the day they change: `AddRange`/`RemoveRange` silently drop the WHOLE range when it ends at `math.MaxUint64` (MEASURED cardinality 0 against a naive 6, with the exact control one id lower); an inverted `AddRange` creates a permanent serialized entry for an empty label (MEASURED 1 000 of them turning a 16-byte image into 20 016 bytes while `Count` reports 0 for every one); and the serialized form is not idempotent in the window [4, 8] where a run container is small enough to be down-converted (MEASURED 55 bytes in, 72 out, converging after one cycle, with 9 and above stable). Twenty-five perturbations each fire their named clause with the control silent, and all fifteen non-vacuity gates are proved fireable by a knockout — including four no configuration knob can reach. See the coverage document |
| `production-profile` (`internal/sim/production_profile.go`) | concurrent | **A realistic multi-client production environment in one command** (rmp #2441): the full role population — contended and disjoint transactions of mixed sizes, atomic batches, during-run oracle readers, RYOW probes, plain writers and readers, and overload traffic — over the durable store, in crash cycles, each cycle joining every client, crashing the disk, reopening through real recovery, and adjudicating the transaction-granular ledgers (acked ⊆ recovered ⊆ issued; a refused transaction leaves nothing; contended counters carry their acknowledged increments across every crash). Since rmp #2469 it runs over the checkpoint-backed layout and takes a checkpoint WHILE its MVCC traffic is in flight, and since rmp #2470 each cycle carries a standing MVCC-substrate watch. Short layer: 24 connections, two cycles; soak: 256 connections, three cycles. See [The production profile](#the-production-profile---scenarioproduction-profile). |
