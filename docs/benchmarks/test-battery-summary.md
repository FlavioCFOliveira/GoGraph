...
**Known limitations:** No automatic N-ops trigger in `Config` (Sprint 67 note). Snapshot v1 lacks `mapper.bin`; recovery on v1 snapshots requires WAL replay. `txn.NewStoreWithCodec` (or `NewStoreWithOptions`) required for `OpAddEdge` in write paths (the v1 `txn.NewStore` was removed, #929).
