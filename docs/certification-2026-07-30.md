...
| **Isolation** | `store/txn`, `store/recovery`, `store/checkpoint`, `store/snapshot` green under `-race`. **Caveat:** the soak-gated isolation tests had never actually run — see #2259. |
| **Durability** | WAL, recovery, checkpoint and snapshot suites green. **One open defect (#2262):** `labels.bin` has no slot ordinal, so a checkpoint round-trip loses or invents relationship types for handle-less parallel edges. Cypher-built graphs are unaffected (types are durable via `edgehandles.bin`). |
