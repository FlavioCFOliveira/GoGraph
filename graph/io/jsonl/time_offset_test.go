...
// (#1769): a PropTime with a non-UTC zone offset must round-trip preserving the
// offset, not normalise to UTC.
...
func TestJSONL_TimeOffset_RoundTrips(t *testing.T) {
	t.Parallel()
	...
	got, _, err := jsonl.ReadWithProps(bytes.NewReader(buf.Bytes()), adjlist.Config{Directed: true})
	if err != nil {
