...
// regression test for NON-string node keys (docs/acid-audit.md).
//
// WriteSnapshotFull only emits mapper.bin (the durable NodeID->key table)
// for string keys; for any other key type it writes a v2 snapshot without
// a mapper, which CANNOT reconstruct the graph on its own. If the
// checkpointer truncated the WAL after such a snapshot, the NodeID->key
// mapping — and therefore every edge/label/property keyed by it — would be
// permanently lost.
//
// runCheckpoint guards against this: it truncates the WAL only when the
// snapshot is self-sufficient (carries mapper.bin). For non-string keys it
// skips truncation, so the WAL is retained and replayed at recovery. This
// test commits int64-keyed edges + a label + a property, checkpoints, and
// asserts:
//
...
//
// i.e. Durability is preserved for non-string keys even though full
// self-sufficient (truncating) checkpoints are not yet available for them.
