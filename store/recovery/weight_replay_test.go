...
// the WAL, then reopen via [Open] and assert the weight survived.
// Covers the two canonical built-in [txn.WeightCodec] instantiations
// required by the rmp #174 acceptance criterion.
...
// then re-opens it with a [txn.WeightCodec] registered via [Open].
// Forward compatibility means the apply path writes the zero value
// of W to the graph for the unweighted records, without erroring on
// the missing weight section.
