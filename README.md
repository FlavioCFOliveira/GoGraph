...
**Current release: `v3.0.0`** (stable). The five major subsystems below are
functional and tested under race, lint, and soak gates. v3.0.0 is a
**security-hardening and distribution** release: it lands a complete,
proof-of-concept-confirmed remediation of a 20-finding security audit and
adopts the conventional Go module path
`github.com/FlavioCFOliveira/GoGraph`, so the library is fetchable with
`go get`. v3.0.0 meets the release gates in
[docs/semver.md](docs/semver.md#release-gates): execution-level openCypher
TCK is **100 % (3 897/3 897 scenarios, 16 006/16 006 steps)** — held across
the audit remediation and the module rename — CI is fully green (build,
test, race, lint, vet, TCK, govulncheck), and all T-series tasks are closed.
The module is **100 % openCypher TCK-compliant at the execution level** and
**100 % ACID-compliant**. Production deployments should pin the `v3.0.0`
tag. See [CHANGELOG.md](CHANGELOG.md) and
[release-notes/v3.0.0.md](release-notes/v3.0.0.md) for the full release
narrative, [docs/migration-2-to-3.md](docs/migration-2-to-3.md) to upgrade
from the 2.x line, and [docs/migration-1-to-2.md](docs/migration-1-to-2.md)
