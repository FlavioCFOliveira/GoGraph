...
// Engine.RunAny, which routes reads through the lock-free read path and
// writes through a durable, serialised transaction. The result is fully
// drained and closed inside the handler, because a write holds the
// store's single-writer mutex until the result is closed.
...
// handleSeed loads the deterministic fixture. It is idempotent: the
// response reports whether data was actually written.
...
// through the engine, reflecting the committed state at request time.
...
// "n". The result is drained and closed before returning.
